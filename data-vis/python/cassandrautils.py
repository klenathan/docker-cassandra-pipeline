...
AQI_TABLE = os.environ.get("AQI_TABLE") if os.environ.get("AQI_TABLE") else 'aqi'
...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + " (name, address, credit_card_expire, credit_card_num, credit_card_provider, date_of_birth, email, job, origin, phone, year) \
                   VALUES (?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ["name", "address", "credit_card_expire", "credit_card_num", "credit_card_provider", "date_of_birth", "email", "job", "origin", "phone", "year"]
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveFakerDf(dfData)
    elif target == 'aqo':
...
def getDF(source_table):
    ...
    if source_table not in (WEATHER_TABLE, TWITTER_TABLE, FAKER_TABLE, AQI_TABLE):
        return None
